...
function createCard(pokeObject) {
  ...
  Card.id = "pokecard";
  Card.className = `max-w-md w-[90%] mx-auto my-10 bg-white rounded-3xl shadow-2xl overflow-hidden ring-4 ring-[${color}] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_40px_-10px_${color}]`;

  const header = document.createElement("div");
  header.className = `relative bg-[${color}] pt-6 pb-14 px-6 flex items-center justify-between`;
  ...
  pokeImg.className = `absolute -bottom-12 right-6 w-28 h-28 object-contain drop-shadow-xl bg-white rounded-full border-4 border-white`;

  const pokeName = document.createElement("h2");
  pokeName.textContent = pokeObject.pokeName;
  pokeName.className = `text-2xl font-extrabold capitalize tracking-wide`;
  ...
  listStats.className = `grid grid-cols-2 gap-x-8 gap-y-3 px-6 pt-16 pb-6 text-sm font-semibold text-[#2A2A2A]`;
